...
const Login = ({ setShowLogin }) => {
	...
	const handleLogIn = async (e) => {
		...
		try {
			...
			const response = await axios.post('http://192.168.2.11:8007/login', {
				email,
				password,
			});
			...
			if (success) {
				...
				const checkRegistered = await axios.post(
					'http://192.168.2.11:8007/regstatus',
					{ email }
				);
				// status codes
				...
				if (registered) {
					...
					const checkID = await axios.post(
						'http://' + HOST_IP_ADDRESS + ':8007/getuserid',
						{ email }
					);
		...
		<>
			<div className="register-background">
				<div className="register-container">
					<div className="exit-button" onClick={exitLogin}>
						x
					</div>
					<h1>Welcome Back!</h1>
					<div className="form">
