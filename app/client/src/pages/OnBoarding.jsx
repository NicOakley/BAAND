...
const OnBoarding = () => {
	...
	// Show text input
	const showTextInput = () => {
		setTextInputClass('onboarding-input-text');
	};

	// Show instrument selection
	const showInstrumentSelection = () => {
		setTitleClass('instrument-selection');
	};

	// Show Plays for selection
	const showPlaysForSelection = () => {
		setTitleClass('instrument-selection');
	};

	// Hide text input
	const hideTextInput = () => {
		setTextInputClass('invisible');
	};

	// Hide instrument selection
	const hideInstrumentSelection = () => {
		setTitleClass('invisible');
	};

	// Hide Plays for selection
	const hidePlaysForSelection = () => {
		setTitleClass('invisible');
	};
	...
	const [instrumentSelectionClass, setInstrumentSelectionClass] =
		useState('invisible');
	const [seekingSelectionClass, setSeekingSelectionClass] =
		useState('invisible');
		...
		if (currentQuestion >= numQuestions - 1) {
			...
			if (
				...
				const response = await axios.post(
					'http://' + HOST_IP_ADDRESS + ':8007/onboard',
					{
						userid: currentUser,
						name: firstName,
						age: age,
					...
					<div className="onboarding-next">
						<button onClick={handleClick}>
							...
							>
								<path
									fill="#0a0a0a"
									d="M39.78 20.768a2.5 2.5 0 0 0 0-3.536L23.872 1.322a2.5 2.5 0 1 0-3.535 3.536L34.477 19 20.335 33.142a2.5 2.5 0 0 0 3.536 3.536l15.91-15.91ZM0 21.5h38.013v-5H0v5Z"
								/>
							</svg>
						</button>
					</div>
